/*****************************************************************************
*  cx_message.c -- support for message objects
*
*  The code in this file is designed to be robust and self-contained.
*
*  The C interface is defined in cx_message.h.
*
...
/*============================================================================
 *  cx_e_message_create_datatypes() -- create message-related datatypes
 *
...
static char *dts[] =  {
   ...
   "MSG",     CX_PROP_MESSAGE,  "Message", "1", "STRLST",
   "Message as sequence of strings",

   "NAM",     CX_PROP_NAME,     "Name",    "1", "STRING",
   "Name as arbitrary ASCII text",

   "FR",      CX_PROP_FROM,     "From",    "1", "STRING",
   "From (e.g., origin) as ASCII text",

   "TO",      CX_PROP_TO,       "To",      "1", "STRING",
   "To (e.g., destination) as ASCII text",
 ...
 *  message_stringvalue()
 *
 *  Local function  -- use cx_stringvalue(msg) externally.
 */

static cx_String message_stringvalue(cx_Object msg)
{
   cx_Object  sos, sob;
   cx_String  str;
   int        cont;
   ...
   sos = cx_stream(msg, CX_OB_STRING);
   for (cont = FALSE; NULL != (sob = cx_next(sos)); cont = TRUE) {

      /*** Messages contain only strings (so far). ***/

      if (CX_OB_STRING == cx_type(sob)) {

         /*** Possibly add string-delimiter. ***/
   ...
   return cx_quote_dup(cx_scratchpad(""));  /* duplicate? */
}

/*============================================================================
 *  message_destroy() -- destroy a message object
 */

static void message_destroy(cx_Object msg)
{
   cx_e_base_destroy(msg);
}
